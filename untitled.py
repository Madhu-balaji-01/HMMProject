...
def viterbi_5(emission_counts, transition_counts, sequence, states):
    ...
    for obs in sequence:
        ...
        for state in states:
            ...
            for state_prev in pi_scores[temp-1].keys():
                ...
                if (log_transition_param == neg_inf) or (log_transition_param == neg_inf):
    ...
    for state_prev in pi_scores[temp-1].keys():
        ...
        if log_transition_param != neg_inf:
            for i in range(len(pi_scores[temp-1][state_prev])):
                    params_dict[tuple(pi_scores[temp-1][state_prev][i][1] + [state_prev])] = pi_scores[temp-1][state_prev][i][0]  + log_transition_param
...
with open('./ES/dev.p69.out', "w", encoding="utf8") as g:
    k = 0
    num_lines = 0
